...
class Test_EntrySummary13():
    randomInvoice = "ABTstTC13_CtnCN_"+ utills.random_invoceGenerator() # random_invoceGenerator() came from utils
    ...
    def test_TC13_ChinaCotton_SimpleEntry_Vessel_Container(self): # Country of origin india/No Duty HTS(NDC)
        self.log.info("----------------Test Case test_TC13_ChinaCotton_SimpleEntry_Vessel_Container Starterd----------------")
        ...
        for r in range(15, 16):
            ...
            self.esf.chinaHTSToAdd()
            self.esf.lineValue(self.linevalueData)
            self.esf.maximizeQtySectionForChina()
            self.esf.addCottonValuesForChina(self.htsqty1Data, self.htsqty2Data)
            ...
            if 'Form saved succesfully!' in self.msg:
                ...
                # self.driver.save_screenshot(".\\screenshots\\" + "test_HTC13_scr.png")  # Screenshot
                self.esf.formSavedConfirmationMsg()
                self.log.error("----Form Not Saved. Test Failed----")

        self.log.info("----------------Test Case test_TC13_ChinaCotton_SimpleEntry_Vessel_Container End----------------")



# pytest -v -s testcases/test_EntryTC13.py
# pytest -v -s testcases/test_EntryTC13.py --browser chrome
# pytest -v -s testcases/test_EntryTC13.py --browser firefox
# pytest -v -s --html=reports\EntryTC13Report.html testcases/test_EntryTC13.py
# pytest -v --html=reports\EntryTC13Report.html testcases/test_EntryTC13.py
# pytest -v --html=reports\EntryTC13Report.html testcases/test_EntryTC13.py --browser chrome   #if in html report if logs are not getting genrated then remove -s and try