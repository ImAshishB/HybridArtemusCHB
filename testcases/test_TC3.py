...
class Test1LTC():
    ...
    def test_HTC3(self): # Country of origin india/No Duty HTS(NDC)
        self.log.info("----------------Test Case test_HTC3 Starterd----------------")
        ...
        for r in range(5, 6):
            ...
            if 'Form saved succesfully!' in self.msg:
                ...
                self.driver.save_screenshot(".\\screenshots\\" + "test_HTC3_scr.png")  # Screenshot
                self.esf.formSavedConfirmationMsg()
                self.log.error("----Form Not Saved. Test Failed----")
                self.lp.logout()
                assert False

        self.log.info("----------------Test Case test_HTC3 End----------------")



# pytest -v -s testcases/test_TC3.py
# pytest -v -s testcases/test_TC3.py --browser chrome
# pytest -v -s testcases/test_TC3.py --browser firefox
# pytest -v -s --html=reports\report.html testcases/test_TC3.py
# pytest -v --html=reports\report.html testcases/test_TC3.py --browser chrome   #if in html report if logs are not getting genrated then remove -s and try