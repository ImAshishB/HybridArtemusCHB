...
class Test_EntrySummary10():
    randomInvoice = "ABTstTC10_Crncy_"+ utills.random_invoceGenerator() # random_invoceGenerator() came from utils
    ...
    def test_TC10_Currency_SimpleEntry_Vessel_Container(self): # Country of origin india/No Duty HTS(NDC)
        self.log.info("----------------Test Case test_TC10_Currency_SimpleEntry_Vessel_Container with Container Starterd----------------")
        ...
        for r in range(12, 13):
            ...
            self.foreignValueData = utills.readData(self.file, 'TcHybridArtemusData', r, 36)
            ...
            self.esf.currency(self.currencyData)
            self.log.info("----Trading Partners 2 Done----")

            # Line Items
            self.log.info("----Line Items Started----")
            self.esf.foreinValue(self.foreignValueData)
            ...
            if 'Form saved succesfully!' in self.msg:
                ...
                # self.driver.save_screenshot(".\\screenshots\\" + "test_HTC10_scr.png")  # Screenshot
                self.esf.formSavedConfirmationMsg()
                self.log.error("----Form Not Saved. Test Failed----")

        self.log.info("----------------Test Case test_TC10_Currency_SimpleEntry_Vessel_Container with Container End----------------")



# pytest -v -s testcases/test_EntryTC10.py
# pytest -v -s testcases/test_EntryTC10.py --browser chrome
# pytest -v -s testcases/test_EntryTC10.py --browser firefox
# pytest -v -s --html=reports\EntryTC10Report.html testcases/test_EntryTC10.py
# pytest -v --html=reports\EntryTC10Report.html testcases/test_EntryTC10.py
# pytest -v --html=reports\EntryTC10Report.html testcases/test_EntryTC10.py --browser chrome   #if in html report if logs are not getting genrated then remove -s and try