...
class Test_EntrySummary11():
    randomInvoice = "ABTstTC11_Cotton"+ utills.random_invoceGenerator() # random_invoceGenerator() came from utils
    ...
    def test_TC11_CottonDontExemptHTS_SimpleEntry_Vessel_Container(self): # Country of origin india/No Duty HTS(NDC)
        self.log.info("----------------Test Case test_TC11_CottonDontExemptHTS_SimpleEntry_Vessel_Container with Container Starterd----------------")
        ...
        for r in range(13, 14):
            ...
            self.esf.cottonHTSNotToExempt()
            self.esf.lineValue(self.linevalueData)
            self.esf.maximizeQtySection()
            self.esf.addCottonValues(self.htsqty1Data,self.htsqty2Data)
            ...
            if 'Form saved succesfully!' in self.msg:
                ...
                TotalOtherFeesOfVlidationForm = self.driver.find_element(By.XPATH,"//p[@class='form-lable'][contains(text(),'Total Duty:')]//span[4]").text
                InvoiceValuesOfValidationFormFloat = float(InvoiceValuesOfValidationForm)
                TotalOtherFeesOfVlidationFormFloat = float(TotalOtherFeesOfVlidationForm)
                if InvoiceValuesOfValidationFormFloat != 0 and TotalOtherFeesOfVlidationFormFloat!=0:
                ...
                # self.driver.save_screenshot(".\\screenshots\\" + "test_HTC11_scr.png")  # Screenshot
                self.esf.formSavedConfirmationMsg()
                self.log.error("----Form Not Saved. Test Failed----")

        self.log.info("----------------Test Case test_TC11_CottonDontExemptHTS_SimpleEntry_Vessel_Container with Container End----------------")



# pytest -v -s testcases/test_EntryTC11.py
# pytest -v -s testcases/test_EntryTC11.py --browser chrome
# pytest -v -s testcases/test_EntryTC11.py --browser firefox
# pytest -v -s --html=reports\EntryTC11Report.html testcases/test_EntryTC11.py
# pytest -v --html=reports\EntryTC11Report.html testcases/test_EntryTC11.py
# pytest -v --html=reports\EntryTC11Report.html testcases/test_EntryTC11.py --browser chrome   #if in html report if logs are not getting genrated then remove -s and try