...
class Test_EntrySummary_Demo():
    randomInvoice = "ABTestTC_Demo"+ utills.random_invoceGenerator() # random_invoceGenerator() came from utils
    ...
    def test_TC_Demo_SimpleEntry_Vessel_Container(self): # Country of origin india/No Duty HTS(NDC)
        self.log.info("----------------Test Case test_TC_Demo_SimpleEntry_Vessel_Container with Container Starterd----------------")
        ...
        for r in range(3, 4):
            ...
            # Save the form
            time.sleep(1)
            self.mywait.until(EC.element_to_be_clickable((By.XPATH, "(//button[@title='Open/Edit Form'])[2]"))).click()
            time.sleep(1)
            self.esf.saveform()
            time.sleep(1)
            ...
            if 'Form saved succesfully!' in self.msg or 'Form updated succesfully!' in self.msg:
                ...
                if InvoiceValuesOfValidationFormFloat != 0:
                    ...
                    print("----The values are calculated properly----")
                    ...
                    print("----The values are not calculated properly----")
            else:
                # self.driver.save_screenshot(".\\screenshots\\" + "test_HTC_Demo_scr.png")  # Screenshot
                self.esf.formSavedConfirmationMsg()
                self.log.error("----Form Not Saved. Test Failed----")

        self.log.info("----------------Test Case test_TC_Demo_SimpleEntry_Vessel_Container with Container End----------------")



# pytest -v -s testcases/test_Entry_Demo.py
# pytest -v -s testcases/test_Entry_Demo.py --browser chrome
# pytest -v -s testcases/test_Entry_Demo.py --browser firefox
# pytest -v -s --html=reports\test_Entry_DemoReport.html testcases/test_Entry_Demo.py
# pytest -v --html=reports\test_Entry_DemoReport.html testcases/test_Entry_Demo.py
# pytest -v --html=reports\test_Entry_DemoReport.html testcases/test_Entry_Demo.py --browser chrome   #if in html report if logs are not getting genrated then remove -s and try